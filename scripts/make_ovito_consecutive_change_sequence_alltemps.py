...
OUT_ROOT = Path.home() / "MD_GB_sims/gb_data_ovito_consecutive_change_sequence_alltemps"
...
def process_temp(temp, pred):
    tmin, tmax = WINDOWS[temp]
    ...
    if len(g) < 2:
        print(f"Skipping {temp} K: not enough frames.")
        return
    ...
    for i, row in g.iterrows():
        ...
        if prev_atoms is None:
            ...
            if len(prev_atoms) != n:
                raise ValueError(f"Atom count mismatch for {temp} K: {prev_timestep} vs {timestep}")
